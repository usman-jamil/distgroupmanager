...
namespace ADManager
{
    public class ADMethodsAccountManagementDefault : IADManager
    {
        ...
        public List<string> GroupMembers(string sGroupName)
        {
                ...
                Environment.UserDomainName, strDN))
            {
                ...
                {
                    using (DirectoryEntry dE = new DirectoryEntry("LDAP://" + oGroupPrincipal.DistinguishedName))
                    {
                        dE.RefreshCache(new string[] { "member" });
                    }
